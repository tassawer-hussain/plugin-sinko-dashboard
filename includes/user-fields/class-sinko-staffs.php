...
class Sinko_Staff extends WP_List_Table {
	...
	public function __construct( ) {
		...
		parent::__construct( [
			'singular' => __( 'Staff', 'sinko-dashboard' ), //singular name of the listed records
			'plural'   => __( 'Staffs', 'sinko-dashboard' ), //plural name of the listed records
     ...
     * @return mixed
     */
    public static function get_staff( $per_page = 20, $page_number = 1 ) {

        $sinko_role = serialize(array( 'sinko_staff' => true ));
    ...
    public function no_items() {
        _e( 'No staff avaliable.', 'sinko-dashboard' );
    }
    ...
    function column_staffs( $item ) {
        ...
        // create a nonce
        $delete_nonce = wp_create_nonce( 'sinko_delete_staff' );
        ...
        $actions = [
            'Edit' => sprintf( '<a href="?page=%s&action=%s&client=%s&_wpnonce=%s">Edit</a>', esc_attr( $_REQUEST['page'] ), 'edit', absint( $item['ID'] ), $delete_nonce ),
            'delete' => sprintf( '<a href="?page=%s&action=%s&client=%s&_wpnonce=%s">Delete</a>', esc_attr( $_REQUEST['page'] ), 'delete', absint( $item['ID'] ), $delete_nonce )
        ];
    ...
    function column_projects( $item ) {
        ...
        // create a nonce
        $edit_nonce = wp_create_nonce( 'sinko_edit_client' );

        $title = sprintf( '<a href="?page=%s&action=%s&client=%s&_wpnonce=%s">%s</a>', esc_attr( $_REQUEST['page'] ), 'edit', absint( $item['ID'] ), count( maybe_unserialize( $client_projects ) ), $edit_nonce );
    ...
    function column_clients( $item ) {
        ...
        $client_staff = get_user_meta( $item['ID'], 'client_assigend' );
    ...
    public function prepare_items() {
        ...
        $per_page     = $this->get_items_per_page( 'staffs_per_page', 20 );
        ...
        $this->items = self::get_staff( $per_page, $current_page );
    ...
    public function process_bulk_action() {
        ...
        if ( 'delete' === $this->current_action() ) {
          	...
          	if ( ! wp_verify_nonce( $nonce, 'sinko_delete_staff' ) ) {
    ...
    public function display_staff_list() { ?>
        <div class="wrap">
            <h2>
                <?php echo __('Staff', 'sinko-dashboard'); ?>
